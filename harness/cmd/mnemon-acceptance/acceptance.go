...
Use these patterns from the workspace root:
  ...
  mnemon-harness view --addr "$MNEMON_CONTROL_ADDR" --principal "$MNEMON_CONTROL_PRINCIPAL" --token-file "$MNEMON_CONTROL_TOKEN_FILE" --lifecycle mid --surface agent
...
func runManualR1HookReminder(agent *r1CodexAgent) (string, error) {
	hook := filepath.Join(agent.workspace, ".codex", "hooks", "mnemon-r1", "mid.sh")
...
func renderR1DerivedEventPresentation(controlURL, token string) (presentation.Response, error) {
	body, _ := json.Marshal(presentation.Request{RenderIntent: presentation.IntentBrief, Lifecycle: "mid", Surface: "hook"})
